class DisplayTasksTabPanel < Netzke::Basepack::TabPanel
  ...
  def items
    task_groups       = TaskGroup.sorted.where(:is_enabled => true)
    ...
    task_groups.each do |task_group|
      ...
      task_group.tasks.sorted.where(:is_enabled => true).each do |task|
        task_item               = Hash.new
        ...
        task_item[:html]        = task.task_remote_commands.sorted.collect{ |trc| "<pre class='display-information'><div id='#{trc.html_display_id}' class='display'></div></pre>" }.join
